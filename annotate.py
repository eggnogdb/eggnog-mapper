...
#        'PFAM': gzip.open(args.output+'.PFAM.txt.gz', "w:gz"),
    ...
    def print_results():
        for k in sorted(query_data):
            hit_details = query_data[k]
            n_ogs = len(set([v[0] for v in hit_details]))
            n_hits = len(hit_details)

            tabprint(OUT[k[0]], query, *(hit_details[0]+list(k)))
            if args.full_report:
        ...
        for go_cat, terms in gos.iteritems():
            for goid, goname, evidence, nseqs, freq, _ in terms:
                if float(freq) >= args.min_prevalence:
                    if ("go", goid, goname) not in query_data or args.full_report:
                        query_data[("go", goid, goname)].append(hitinfo + [nseqs, freq])
