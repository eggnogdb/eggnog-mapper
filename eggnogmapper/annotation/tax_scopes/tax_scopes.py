...
def parse_tax_scope(tax_scope):
    ...
    if tax_scope_ids is not None and len(tax_scope_ids) > 0:
        ...
        for tax_id in tax_scope_ids:
            ...
            elif _taxid_exists_in_db(tax_id):
                tax_scope_ids_int.append(tax_id)
            else:
...
##
def _parse_og_name(nog):
    """Parse an OG name into (nog_id, nog_tax_id).

    Handles both v5 and v7 OG name formats:
      v5: "COG0012@1"        → ("COG0012", "1")
      v7: "UNK.278C@131567|A-1" → ("UNK.278C", "131567")
    """
    parts = nog.split("@", 1)
    nog_id = parts[0]
    tax_part = parts[1] if len(parts) > 1 else "1"

    # v7 format: taxid|clade_code — strip the clade code
    if "|" in tax_part:
        nog_tax_id = tax_part.split("|", 1)[0]
    else:
        nog_tax_id = tax_part

    return nog_id, nog_tax_id
...
def _taxid_exists_in_db(tax_id):
    """Check if a taxid exists in eggnog.taxa.db."""
    try:
        from ...common import get_ncbitaxadb_file
        import sqlite3
        conn = sqlite3.connect(get_ncbitaxadb_file())
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM species WHERE taxid = ?", (int(tax_id),))
...
def _name_from_taxa_db(nog_tax_id):
    """Get taxon name from taxa DB, with cache."""
    if nog_tax_id in _taxa_name_cache:
        return _taxa_name_cache[nog_tax_id]
    try:
        from ...common import get_ncbitaxadb_file
        import sqlite3
        conn = sqlite3.connect(get_ncbitaxadb_file())
        cur = conn.cursor()
        cur.execute("SELECT spname FROM species WHERE taxid = ?", (int(nog_tax_id),))
        row = cur.fetchone()
        conn.close()
        if row and row[0]:
            _taxa_name_cache[nog_tax_id] = row[0]
            return row[0]
    except Exception:
        pass
    return nog_tax_id  # fallback to numeric ID


def _get_depth(nog_tax_id):
    """Get depth for a taxonomic level, with fallback for unknown taxids.

    Uses the hardcoded v5 LEVEL_DEPTH first. For v7 taxids not in that dict,
    attempts to compute depth from the taxonomy DB (lineage length).
    Falls back to a large number so unknown taxids sort as narrow/specific.
    """
    if nog_tax_id in LEVEL_DEPTH:
        return LEVEL_DEPTH[nog_tax_id]

    if nog_tax_id in _taxa_depth_cache:
        return _taxa_depth_cache[nog_tax_id]

    # Try to get depth from taxa DB
    depth = _depth_from_taxa_db(nog_tax_id)
    if depth is not None:
        _taxa_depth_cache[nog_tax_id] = depth
        return depth

    # Fallback: assign a high depth so it sorts as narrow/specific
    _taxa_depth_cache[nog_tax_id] = 100
    return 100


def _depth_from_taxa_db(nog_tax_id):
    """Compute depth from eggnog.taxa.db lineage track."""
    try:
        from ...common import get_ncbitaxadb_file
        import sqlite3
        taxa_file = get_ncbitaxadb_file()
        conn = sqlite3.connect(taxa_file)
        cur = conn.cursor()
        cur.execute("SELECT track FROM species WHERE taxid = ?", (int(nog_tax_id),))
        row = cur.fetchone()
        conn.close()
        if row and row[0]:
            # track is a comma-separated lineage of taxids; depth = length
...
def parse_nogs(match_nogs, tax_scope_mode, tax_scope_ids):
    ...
    for nog in match_nogs:
        nog_id, nog_tax_id = _parse_og_name(nog)
        level_name = LEVEL_NAMES.get(nog_tax_id) or _name_from_taxa_db(nog_tax_id)
        nog_name = f"{nog}|{str(level_name).replace(',', ' ')}"

        match_nogs_full.append((nog_id, nog_tax_id, nog_name, _get_depth(nog_tax_id)))
    ...
    if tax_scope_ids is None:
        ...
        # choose narrowest OGs
            ...
            if tax_scope_mode_ids is not None:
                ...
                if len(inters) > 0:
                    max_depth = max([_get_depth(tax_id) for tax_id in inters])
    ...
    # Obtain best OG based on tax scope
        ...
        if len(inters) > 0:
            ...
            if tax_scope_mode == TAX_SCOPE_MODE_BROADEST:
                ...
                min_depth = min([_get_depth(tax_id) for tax_id in inters])
                candidate_best_ogs = [nog for nog in match_nogs_full if nog[3] == min_depth]
